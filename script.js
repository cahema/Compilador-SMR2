...
function generar() {
	...
	procActual.banderas.instruccionIlegal = false;
	procActual.banderas.registroIlegal = false;
	procActual.banderas.numeroIlegal = false;
	
	while (contador < arrCodigo.length) {
		
		comprobarErrores();
		lineaError++;
		let instruccionActual = procActual.diccionarios.instrucciones[arrCodigo[contador]]; //Se guarda el objeto con la instruccion actual
		
		if (instruccionActual == undefined) { procActual.banderas.instruccionIlegal = true; continue; } //Si es undefined activa la bandera de instruccionIlegal
